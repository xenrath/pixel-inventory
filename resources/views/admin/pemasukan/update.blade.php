...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            @endif
            <form action="{{ url('admin/pemasukan/' . $pemasukan->id) }}" method="POST" autocomplete="off">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-6">
    ...
    <script>
        ...
        var supplier_id = @json(old('supplier_id', $pemasukan->supplier_id));
        if (supplier_id) {
    ...
    <script>
        ...
        var user_id = @json(old('user_id', $pemasukan->user_id));
        if (user_id) {
            sales_set(user_id);
        }
    </script>
    <script>
        ...
        var grand_total = @json(old('grand_total', $pemasukan->grand_total));
        if (grand_total) {
            $('#span-grand-total').text(rupiah("" + grand_total, "Rp"));
            $('#grand-total').val(grand_total);
        }
